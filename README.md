...
**v0.4** 优化生成解析内容
优化生成解析内容，简化解析内容，保留必须项和确定结论，对因果方面混淆进行区分。方便提取md文件做整理。

**v0.5** **已更新** 新增自动读取DOI、作者、年份的功能。新增支持查询期刊IF、分区功能。可以在Setting里输入Easy Scholar Key进行配置，快去试试吧！
**9_13更新v0.51** 发现更新API key 会清除Easy Scholar 配置的问题，已修复。
